# Verify 체계 (서브 에이전트 위임)

verify 시 메인 컨텍스트에서 파일을 직접 읽지 말 것. `Task(subagent_type="general-purpose", run_in_background=true)` 백그라운드 병렬 위임으로 토큰 효율화 (메인 컨텍스트 96% 절감). 에이전트 완료 알림을 받아 결과를 취합한다.

**공통 규칙**:
- 각 에이전트는 불일치 목록만 반환, 메인은 취합 후 보고/수정
- Read 우선 비교: 비교 소스를 먼저 Read한 뒤 메모리 내에서 비교한다. 항목별 개별 검색보다 전체 Read를 우선한다
- **임시 파일 RAII**: 중간 파일 생성 허용하되, 에이전트 종료 전 반드시 삭제 (생성한 파일 목록 추적 → 최종 단계에서 정리)
- **재위임 금지**: 에이전트 내부에서 Task 도구로 서브에이전트를 재생성하지 말 것. 직접 Read/Grep으로 확인
- **파일 읽기 필수**: 반드시 Read/Grep 도구로 실제 파일을 읽어서 비교해야 한다. 파일을 읽지 않고 결과를 추측/생성하면 안 된다. 종료 시 사용한 Read/Grep 호출 수를 보고한다
- `npm test`는 bb_test에 해당하며 func_verify 이상에서 전체 실행
...
2. ALL PASS → 에이전트는 스크립트 미커버 영역의 **의미적 확인만** 수행
3. FAIL → 불일치 항목을 에이전트에 전달하여 원인 분석 위임

**에이전트 부하 절감**:
- B 에이전트: CSS 변수 전수 비교를 스크립트가 대체 → 컴포넌트/반응형/애니메이션 의미적 확인에 집중
- A2 에이전트: ARIA/접근성 전수 확인을 스크립트가 대체 → 문서 정확성 + 구현 준수 확인에 집중
...
| **wb_test** | 소스코드를 파싱/비교하여 확인 | `npm run verify` (Tier 0), Tier 2 에이전트 |

## Verify 수준

각 단계는 하위 단계를 **모두 포함**한다 (strong ⊃ func ⊃ weak).

| 수준 | 별칭 | 누적 구성 | 에이전트 | 테스트 타입 | 용도 |
|------|------|----------|---------|-----------|------|
| **weak_verify** | 약한 검증 | Tier 0 + `npm test:logic` | 없음 | wb + bb | 로직 변경 빠른 확인 |
| **func_verify** | 기능 검증 | **weak 전체** + `npm test:dom` + serve.js E2E | 없음 | wb + bb | 기능/UI 변경 확인 |
| **strong_verify** | 강한 검증 | **func 전체** + Tier 2 에이전트 | 5개 (백그라운드) | wb + bb | 문서 변경 / 릴리스 전 |

- func_verify의 serve.js E2E는 MCP 환경(Desktop)에서 실행. CLI에서는 `npm test:dom`까지 실행

## Verify Tier

| 계층 | 방식 | 테스트 타입 | verify 수준 |
|------|------|-----------|----------|
| **Tier 0** | 스크립트 자동 체크 (`verify.js`) | wb_test | weak 이상 |
| **Tier 1** | 블랙박스 — `npm test` + serve.js E2E | bb_test | func 이상 |
| **Tier 2** | 화이트박스 — 소스 ↔ 명세 전수 비교 | wb_test | strong |
...
| 원칙 | 설명 |
|------|------|
| 런타임 균형 | 에이전트 간 실행시간이 균형을 이루도록 작업량을 배분한다 |
| 소스 겹침 최소화 | 같은 파일을 여러 에이전트가 반복 읽지 않도록 분할한다 |
| 후처리 | 완료 후 특정 에이전트가 지나치게 오래 걸렸다면 원인을 분석하고 다음 verify에서 분할/병합을 조정한다 |

## A. 문서 비교 (문서 ↔ 소스코드)

**Tier 1** (1 에이전트, ~20K): Tier 0(`npm run verify`) 결과를 기준으로 스팟체크. 함수/테스트/파일 카운트는 Tier 0이 커버
...
> A2: spec.md 문서 정확성 + F-001~F-008 구현 준수 + ARIA/접근성 양방향 확인. 소스를 한 번만 읽고 양쪽 수행
> A3: README.md 파일 트리 ↔ Glob 실제 파일, test/README.md ↔ 테스트 코드, CLAUDE.md 모듈 테이블 ↔ 실제 구조 교차 비교

<agent-prompt id="A1">
# A1: tech.md ↔ 소스코드 전수 비교
...
- **파일 읽기 필수**: 반드시 Read 도구로 비교 소스의 모든 파일을 읽어야 한다. 파일을 읽지 않고 결과를 추측/생성하면 안 된다.
- **Read 우선 비교**: 비교 소스를 먼저 Read한 뒤 메모리 내에서 비교한다. 항목별 개별 검색보다 전체 Read를 우선한다.
- 종료 시 사용한 Bash 명령어 목록과 **Read/Grep 호출 수**를 출력한다.
...
기존 텍스트 출력 후 반드시 아래 JSON 블록을 **마지막에** 출력한다:
...
```json
{
  ...
  "mismatchCount": 0,
  "mismatches": [
    { "file": "파일명", "line": "줄번호", "expected": "기대값", "actual": "실제값", "desc": "설명" }
  ],
  "detail": "ALL PASS 또는 요약"
}
```

- `status`: 불일치 0건이면 `"pass"`, 1건 이상이면 `"fail"`
- `mismatches`: 불일치 목록. ALL PASS 시 빈 배열 `[]`
- `detail`: 기존 텍스트 출력의 첫 줄 ("불일치: N건" 또는 "ALL PASS")
</agent-prompt>

**A2 프롬프트 설계** (ADR-024, ADR-028):
- 체크 1: spec.md가 소스를 정확히 기술하는지 (문서 정확성)
- 체크 2: 소스가 F-001~F-008 요구사항을 구현하는지 (구현 준수)
- 체크 3: ~~ARIA/접근성 속성 전수 확인~~ → **Tier 0 이관** (aria.attributes + aria.srOnly). Tier 0 ALL PASS 시 A2는 ARIA 확인 생략
- **Bash 금지**: Read + Grep만 사용
- **금지 패턴**: (1) Glob 금지 — 파일 경로 고정 (2) ARIA 속성 수동 확인 금지 — Tier 0 위임

<agent-prompt id="A2">
# A2: spec.md ↔ 소스코드 양방향 비교
...
Tier 0 ALL PASS → A2는 ARIA 확인 생략.
...
- DOM 테스트 카운트 규칙: **고유 테스트 케이스 수** 기준 (중복 카운트 주의)
...
<agent-prompt id="A3">
# A3: CLAUDE.md + README.md + test/README.md ↔ 실제 구조 전수 비교
...
- 재위임 금지: Task 도구로 서브에이전트를 재생성하지 않는다.
- 기대값 하드코딩 금지: 문서에서 기대값을 파싱하여 소스와 비교한다.
- **파일 읽기 필수**: 반드시 Read/Glob 도구로 비교 소스의 모든 파일을 읽어야 한다. 파일을 읽지 않고 결과를 추측/생성하면 안 된다.
- 종료 시 사용한 Bash 명령어 목록과 **Read/Glob 호출 수**를 출력한다.

## 체크 항목 (행위 기반 3단계)

### 단계 1: CLAUDE.md 모듈 구조 확인

- CLAUDE.md 모듈 구조 테이블에서 모듈 수와 각 모듈명을 추출.
- **supabase-config.js는 설정 파일이며 모듈 아님** (CLAUDE.md 모듈 테이블 기준).

### 단계 2: README.md 파일 구조 확인
...
- Glob("{*.*,css/*,config/*,js/*,test/*,docs/*,scripts/**/*}"): 프로젝트 파일 목록 (~35개, .git 등 제외)
...
1. 다음을 **모두 동시에** 호출 (병렬):
   - Read: CLAUDE.md, README.md, test/README.md, package.json
   - Glob("{*.*,css/*,config/*,js/*,test/*,docs/*,scripts/**/*}")
...
- 체크 1: design.md가 실제 디자인을 정확히 기술하는지 (문서 정확성)
- 체크 2: 변수값 + 컴포넌트 + 반응형 + 애니메이션 전수 비교 (구현 일치)
- **Bash 금지**: Read + Grep만 사용
- design.md에서 각 카테고리(변수, 컴포넌트, 반응형, 애니메이션) 명세를 추출하여 style.css·index.html과 전수 비교. CSS 변수값 비교는 Tier 0이 완료하므로 의미적 확인에 집중
- **금지 패턴**: Glob 금지 — 파일 경로는 `docs/design.md`, `css/style.css`, `index.html` 고정
...
Tier 0 ALL PASS → B는 변수값 전수 비교를 생략하고 의미적 확인에 집중.
...
## C. 구현 확인 (spec.md ↔ 실제 동작)

**Tier 1**: Tier 0이 함수 카운트(`functions.total`) + ARIA(`aria.attributes`, `aria.srOnly`)를 커버. Tier 0 PASS 시 별도 에이전트 불필요.

**Tier 2**: A2에 통합 (ADR-024). A2가 spec.md ↔ 소스코드 양방향 비교(문서 정확성 + 구현 준수) 수행.

## D. 테스트 비교 (test/README.md ↔ 실제 테스트)

**Tier 1** (1 에이전트, ~15K): `npm test` 실행 → 전 테스트 통과, 0 fail 확인

**Tier 2** (1 에이전트, Sonnet, ~78K): CLI + DOM/UI 테스트 통합 비교 (ADR-024, ADR-026)
...
- **Bash 금지**: Read + Grep만 사용
- **금지 패턴**: (1) 추가 Glob 금지 — 파일 경로 고정 (2) Bash로 파일 목록 조회 금지

<agent-prompt id="D">
# D: test/README.md ↔ 테스트 코드 통합 비교
...
1. 추가 Glob 금지 — 파일 경로는 위에 고정.
2. **Bash로 파일 조회 금지** — `ls`, `find`, `Get-ChildItem` 등 사용 금지. 파일 확인은 반드시 Read/Grep 도구 사용.
...
- **파일 읽기 필수**: "반드시 Read 도구로 비교 소스의 모든 파일을 읽어야 한다. 파일을 읽지 않고 결과를 추측/생성하면 안 된다" 명시 필수. Read/Grep 호출 수 보고 필수
- **교차 비교 위임**: 에이전트 간 중복 파일 읽기 → 이미 읽는 에이전트에 확인 위임 (ADR-022)
...
strong_verify 성공 시 이 문서 하단 "strong_verify 결과" 섹션을 업데이트:

1. **체크 항목 테이블**: 각 에이전트 결과(ALL PASS / Warning N건)
2. **성능 지표 테이블**: Tool 호출 수, 토큰, 경과 시간, Warning 수
3. **헤더**: 마지막 verify 날짜 + 커밋 해시
...
> 마지막 verify: 2026-03-02 (`101612b`) — **19/21 PASS, 2 FAIL** (전체 에이전트 실행, parallel 모드)
...
| 2 | A1 (Sonnet) | tech.md ↔ 소스코드 | **ALL PASS** | 함수 목록·CSS 트리·HTML ID 전수 일치 |
| 2 | A2 (Sonnet) | spec.md ↔ 소스코드 | **ALL PASS** | 문서 정확성 + F-001~F-008 구현 준수 |
| 2 | A3 (Sonnet) | CLAUDE.md + README ↔ 실제 구조 | **ALL PASS** | 모듈 구조·파일 트리·테스트 수치 전수 일치 |
| 2 | B (Sonnet) | design.md ↔ CSS/HTML | **FAIL 1건** | 다크모드 .number color 오버라이드 design.md 누락 |
| 2 | D (Sonnet) | test/README ↔ 테스트 코드 | **FAIL 1건** | Phase 4 미테스트 함수 6개 이름 미나열 |

**불일치 상세:**
1. B: `docs/design.md` L51-76 — 다크모드 색상 테이블에 `.number { color: #ffffff }` 오버라이드 미기재 (CSS L87-89에 존재) — 문서: design.md 수정
2. D: `test/README.md` L181-183 — Phase 4 미테스트 함수 6개(loadConfig, toggleAuthForm, handleSignIn, handleSignUp, handleSignOut, updateAuthUI) 이름 미나열 — 문서: test/README.md 수정
...
> parallel 모드 (5개 서브에이전트 병렬), 최대 111s
