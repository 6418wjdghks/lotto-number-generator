...
- 기존 용어 매핑: 보통 검증 → weak_verify, 정밀 검증 → strong_verify

## 검증 계층 (Tier)

| 계층 | 방식 | 테스트 타입 | 검증 수준 |
|------|------|-----------|----------|
| **Tier 0** | 스크립트 자동 검증 (`verify.js`) | wb_test | weak 이상 |
...
| 후처리 | 완료 후 특정 에이전트가 지나치게 오래 걸렸다면 원인을 분석하고 다음 검증에서 분할/병합을 조정한다 |
...
> 마지막 검증: 2026-03-01 (`cfdacd6`) — **20/21 PASS, 1 FAIL** (전체 에이전트 실행)
