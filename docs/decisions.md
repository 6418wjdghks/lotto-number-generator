...
### 맥락
기존 테스트(`test/test.html`)는 브라우저에서만 실행 가능하여 자동화가 어려웠다. 사용자가 직접 브라우저를 열고 결과를 확인해야 했으며, CI 파이프라인 연동이 불가능했다.

### 결정
2계층 테스트 구조를 도입한다:
- **계층 1** (`test/test-logic.js`): Node.js `node:test`로 순수 로직 테스트 (CLI 실행)
- **계층 2** (`test/test.html`): 브라우저에서 DOM/UI 포함 전체 테스트 (기존 유지)

### 사유
- **CI 연동**: `node --test test/test-logic.js`로 exit code 0/1 반환. GitHub Actions 등에서 자동 실행 가능
- **외부 의존성 없음**: Node.js 18+ 내장 `node:test` 사용. npm install 불필요
- **빠른 피드백**: CLI 테스트는 브라우저 없이 즉시 실행 (~1초)
- **기존 투자 보존**: 브라우저 테스트(27개)를 버리지 않고 DOM/UI 검증용으로 유지
- **관심사 분리**: 순수 로직(생성, UUID, 이력)은 CLI, DOM 의존 기능(렌더링, 토스트, 클립보드)은 브라우저

### 검토한 대안
- **Puppeteer/Playwright**: 브라우저 자동화로 test.html을 CLI에서 실행. 강력하지만 외부 의존성이 크고(npm), 프로젝트의 "프레임워크 금지" 원칙과 충돌
- **Jest + jsdom**: 풍부한 기능이지만 npm 의존성 필요. Node.js 내장 테스트로 충분
- **브라우저 테스트만 유지**: CI 연동 불가, 자동화 불가능

### app.js 호환성
`module.exports` 가드로 브라우저/Node.js 이중 호환 유지:
```javascript
...
### 맥락
문서 점검 결과 17건의 명세-구현 불일치가 발견되었다. 근본 원인 분석에서 3가지 구조적 결함이 드러났는데, 그 중 하나가 **파생 수치 중복**(6건)이었다. 코드에서 파생되는 수치(함수 수, 테스트 수, 파일 줄 수)가 여러 문서에 중복 기록되어, 코드 변경 시 모든 곳을 동시에 업데이트해야 하는 부담이 발생했다.

예시:
- 함수 수: tech.md(13개)와 CLAUDE.md(17개)가 서로 불일치
- 테스트 수: plan.md(21개, 22개), tech.md(21개)가 test/README.md(23+11)와 불일치
- 파일 줄 수: plan.md에 기록된 줄 수가 현재 코드와 불일치

ADR-011에서 "단일 진실 소스" 원칙을 세웠지만, 파생 수치에는 적용하지 않은 것이 맹점이었다.

### 결정
코드에서 파생되는 수치는 단일 소스에만 기록한다. 다른 문서에서는 수치 대신 해당 소스를 참조한다.
...
### 사유
- **동기화 부담 제거**: 수치 변경 시 1곳만 수정하면 됨
- **정합성 보장**: 중복이 없으므로 문서 간 불일치가 원천 방지됨
- **ADR-011 확장**: 기존 "단일 진실 소스" 원칙을 파생 수치까지 일관되게 적용
