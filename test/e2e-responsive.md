...
- overflow 확인: `document.documentElement.scrollWidth <= document.documentElement.clientWidth`
...
```json
{
  ...
  "items": [
    { "id": "A1", "name": "추첨하기 (1세트)", "status": "pass|fail", "detail": "확인 내용", "screenshotFile": "파일명 또는 null" }
  ],
...
Playwright MCP를 사용하여 특정 뷰포트에서 레이아웃을 확인한다.
...
2. `http://localhost:8080` 으로 이동
3. `browser_evaluate`로 `localStorage.clear()` 실행 후 페이지 새로고침
4. B 그룹 확인 전 추첨 1회 실행 (볼 배치 확인용)
5. 제외 패널 열기 (그리드 배치 확인용)
6. 각 항목의 CSS 속성을 `browser_evaluate` + `getComputedStyle`으로 확인
...
```json
{
  ...
  "viewport": {{VIEWPORT}},
  "items": [
    { "id": "B1-1", "name": "헤더", "status": "pass|fail", "detail": "확인 내용", "screenshotFile": "파일명 또는 null" }
  ],
  "summary": { "total": 0, "pass": 0, "fail": 0 }
}
```
...
Playwright MCP를 사용하여 다크 모드 + 뷰포트 교차 조합을 확인한다.
...
```json
{
  ...
  "items": [
    { "id": "C1", "name": "320px + 다크 모드", "status": "pass|fail", "detail": "확인 내용", "screenshotFile": "파일명 또는 null" }
  ],
